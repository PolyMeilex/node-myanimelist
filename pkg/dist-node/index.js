...
var properUrlJoin = _interopDefault(require('proper-url-join'));
var esmFetch = _interopDefault(require('esm-fetch'));
...
const jikanUrl = "https://api.jikan.moe/v3"; // @ts-ignore
...
class Search {
  ...
  search(type, params) {
    let qparams = Object.keys(params).filter(k => params[k] != null).map(k => `${k}=${encodeURIComponent(params[k])}`).join("&");
...
class TopSimple {
  ...
  topGet(p, subType) {
    ...
    if (p != null) params[1] = String(p);else params[1] = "1";
    params[2] = subType;
...
class UserAnimelist {
  constructor(baseUrl, p) {
    this.baseUrl = baseUrl;
    if (p != null) this.page = String(p);else this.page = "1";
  }
